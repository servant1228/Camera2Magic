...
Kotlin 2.4（AGP 9 内置，不加独立 kotlin 插件）+ kotlinx.serialization。UI：Compose + Miuix 0.9.4-rc01（`-ui/-squircle/-icons/-blur/-preference` + `-nav`）+ navigation3 + material-icons-extended。播放：media3-exoplayer（自定义 DataSource）。Hook：libxposed `api:102` compileOnly + `service:102` implementation。其他：hiddenapibypass、kotlinx-collections-immutable。

**依赖版本唯一真源 = [gradle/libs.versions.toml](gradle/libs.versions.toml)**，坐标/SDK 在 [app/build.gradle](app/build.gradle)；新增依赖一律进 catalog，不在 build.gradle 硬编码版本号（现存少数硬编码条目是历史遗留，触及时顺手迁入 catalog）。SDK：compileSdk 37 / minSdk 34 / targetSdk 36，jvmTarget 21（miuix-nav 的 inline API 要求）。
