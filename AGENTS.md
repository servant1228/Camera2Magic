...
- **`Camera3.start()` 换媒体时分两种路径**：同类型（图→图、视频→视频）在活管线上原地换源；**类型变了（前置图 / 后置视频这种搭配）必须先 `stop()` 再起**——`ExoPlayer` 与 `lockHardwareCanvas` 两种生产者不能混挂在同一个 Surface 上。实现上是 `stop()` 后把 `openMedia` 再 post 一次，靠同一 handler 的 FIFO 保证「拆完再起」（`stop()` 自己也是 post 的，不能在当次 block 里顺序执行）。`activeType` 只在拿到 pfd 后才记，否则失败的下一次起仍是原地换源。**不要在 `start()` 里改成直接串流水**，那是混生产者的入口。
...
**权限面很窄**：唯一权限是 `QUERY_ALL_PACKAGES`（作用域列表要用）。**不加 `CAMERA`**：镜头枚举（`getCameraIdList`/`getCameraCharacteristics`）本来就不要权限（`CameraInventory`/`DeviceInfoSection` 只数 `getCameraIdList()` 的 id 个数并在主页显示一个数字，纯诊断；它**故意不做镜头分类与「几颗镜头」的推断**，原因见该文件 KDoc），而需要权限的那部分特征键（`LENS_INFO_*` 一类，各机型的受限集合由 HAL 声明，只能运行时用 `getKeysNeedingPermission()` 判定）**在 Hook 进程里用目标应用的身份读得到**——所以焦距级别的镜头识别一律留在 Hook 侧，不要为此给模块声明 CAMERA（对一个虚拟摄像头模块来说，那本身就是检测指纹）。**没有任何存储权限**，也没有 `requestLegacyExternalStorage`——媒体只从 Photo Picker 来，再经 `openRemoteFile` 拷进模块私有目录。别为了「读文件」去加存储权限。manifest 里也**没有** Xposed 的 legacy `meta-data`（API 102 用 `META-INF/xposed/*`），别去「补」。预测性返回同样**不在 manifest**：是 `HiddenApiBypass` + 反射 `ApplicationInfo.setEnableOnBackInvokedCallback`，只能在 `onCreate` 生效，这才是切换该开关必须重建 Activity 的原因。
