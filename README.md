...
- 媒体源
  - 按应用配置照片 / 视频（本地媒体）
  - 按应用配置网络视频流（RTSP / HLS / DASH / HTTP(S) 直链），地址即填即用、无需拷贝
- 精细化配置
  - 按应用独立开关「启用 Hook」+ 媒体类型三选一（照片 / 视频 / 网络视频流）；不选媒体即不替换画面
  - 手动旋转
  - 播放声音、Toast、日志开关
- 界面
...
2. 在模块的“作用域”页进入目标应用配置，开启“启用 Hook”，选择“照片/视频/网络视频流”：本地媒体用相册选择器选取，网络视频流则填入 `rtsp://` / `http://` / `https://` 地址（HLS 的 `.m3u8` 与 DASH 的 `.mpd` 走 `http(s)://`）；
3. 进入目标应用即可生效（未配置媒体时不替换画面）。

> 注意：媒体经系统相册选择器选取后会被拷贝进模块私有目录（本模块不申请任何存储权限）；拍照替换仅对支持 JPEG/YUV 输出的相机路径生效。
>
> 网络视频流在**目标应用进程内**拉流，因此需要目标应用自身具备 `INTERNET` 权限；`http://` 明文流还会受目标应用自己的 `networkSecurityConfig` 限制（`rtsp://` 不受影响）。协议支持依赖 media3 的 RTSP / HLS / DASH 模块。
