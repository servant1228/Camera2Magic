...
// Ported from miuix `example/component/liquid/LiquidGlassNavigationBar.kt`.
// Local delta vs upstream: extra `isDark` / `showLabels` params (IconOnly 模式) and the
// height/icon-size branch they drive.
...
import com.nothing.camera2magic.ui.component.animation.DampedDragAnimation
import com.nothing.camera2magic.ui.component.animation.InteractiveHighlight
...
internal fun IosLiquidGlassNavigationBar(
    ...
    val containerHeight = if (showLabels) 64.dp else 56.dp
    val indicatorHeight = if (showLabels) 56.dp else 48.dp
            ...
            Column(
                ...
                BadgedBox(badge = { badge(index)?.invoke() }) {
                    Icon(
                        ...
                        // With labels the Text names the item; icon-only needs the description.
